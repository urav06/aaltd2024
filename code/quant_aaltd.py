...
class QuantClassifier():
    ...
    def _set_num_estimators(self, num_batches: int) -> NDArray[np.int32]:
        ...
        estimators_per_batch: int                   = max(1, int(self.num_estimators / num_batches))
        num_estimators_per_batch: NDArray[np.int32] = np.ones(num_batches, dtype=np.int32) * estimators_per_batch
        total_assigned: int                         = num_estimators_per_batch.sum()
        remaining: int                              = self.num_estimators - total_assigned
